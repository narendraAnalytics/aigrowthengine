...
export const THREATS: readonly Threat[] = [
  // --- assessment_intake ---
  {
    ...
    stride: "tampering",
    threat:
      "Prompt injection in free-text answers steers the classifier or leaks the system prompt.",
    mitigation:
  ...
  },
  {
    ...
    mitigation:
      "assessment.submitted / assessment.scored audit events with immutable payload snapshot + scoring_model_version.",
    status: "needs_review",
  },
  ...
  },
  {
    ...
    mitigation:
      "File records carry organization_id; presign path re-checks TenantScope + rbac; deny by default.",
    status: "needs_review",
  },
  {
    component: "file_access",
    stride: "tampering",
    threat:
      "Malicious upload (malware, oversized, wrong type) processed downstream.",
    mitigation:
      "Type + size limits; content scanning before processing; store originals immutably; process copies.",
    status: "needs_review",
  },
  ...
  // --- investor_room ---
  {
    ...
    stride: "elevation_of_privilege",
    threat:
      "A `registered` investor views `restricted` materials (cap table, contracts).",
    mitigation:
      "meetsInvestorAccess() checked at each document; access level stored server-side; grants are explicit + audited.",
    status: "needs_review",
  },
  {
    component: "investor_room",
    ...
    mitigation:
      "investor_room.access_granted/revoked/document_viewed audit events; 7-year retention.",
    status: "needs_review",
  },
  {
    component: "investor_room",
    ...
    mitigation:
      "Short presigned TTLs; revocation invalidates sessions; watermarking of sensitive docs; no client-side caching.",
    status: "needs_review",
  },
  ...
  },
  {
    ...
    stride: "tampering",
    threat:
      "Model output used as authoritative (a score, a capability id it invented).",
    mitigation:
  ...
  },
  {
    ...
    mitigation:
      "Per-request token ceilings; timeouts; circuit breaker; ai_gateway.request audit with token counts; budget alerts.",
    status: "needs_review",
  },
  {
    component: "ai_gateway",
    ...
    mitigation:
      "ai_gateway.request / ai_gateway.blocked audit events (model, purpose, tokens, outcome).",
    status: "needs_review",
  },
];
